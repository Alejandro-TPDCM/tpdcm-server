...
PAIR_CONFIG = {
    ...
    # ═══════════════════════════════════════════════════════════════════
    # v2.6.0-beta-fixed17: FASE 1 - 6 pares nuevos de 4-decimales
    ...
    'AUD_USD': {
        ...
        'extra_caution_days':    [],
        'block_regimes_always':  [],
        # v2.6.0-beta-fixed17: AUD solo opera London (NY pierde -$5,832)
    ...
    'USD_CAD': {
        ...
        'extra_caution_days':    [],
        'block_regimes_always':  [],
        # v2.6.0-beta-fixed17: CAD solo opera London (NY pierde -$1,365)
    ...
    # ═══════════════════════════════════════════════════════════════════
    # v2.6.0-beta-fixed17: FASE 2 - pares volatiles (JPY + oro)
    ...
    'GBP_JPY': {
        ...
        'extra_caution_days':    [],
        'block_regimes_always':  [],
        # v2.6.0-beta-fixed17: GBP/JPY opera TODAS las sesiones (decision usuaria)
...
MIN_HOURS_BETWEEN_TRADES  = 2   # v2.6.0-beta-fixed17: 3->2 (ajuste fino, mas trades)
...
app = FastAPI(title='TPDCM-IA', version='2.6.0-beta-fixed17')
...
async def fetch_ff_calendar_full():
    """v2.6.0-beta-fixed17: trae TODOS los eventos de la semana (todas las
...
async def run_analysis_pair(pair, auto_execute=False, all_news=None):
    ...
    # v2.6.0-beta-fixed17: filtro de killzones permitidas por par (en vivo)
...
async def run_backtesting_pair(pair, ignore_killzone=False):
    """
    v2.6.0-beta-fixed: Backtest para un par especifico.
    Reutiliza run_ict_pipeline() para no duplicar logica de analisis.
    v2.7: ignore_killzone=True permite probar TODAS las sesiones (investigacion),
    sin tocar la config del par (allowed_killzones).
    """
    pair_cfg = get_pair_config(pair)
    display = pair_cfg['display']
    ...
    try:
        ...
        for i in range(50, len(h1) - 26):
            ...
            if i - last_idx < 4:   # v2.6.0-beta-fixed17: 5->4 velas (ajuste fino)
                cnt['cooldown'] += 1
                continue
            ...
            # v2.6.0-beta-fixed17: solo operar en killzones permitidas (si se define)
            ...
            pair_allowed_kz = pair_cfg.get('allowed_killzones', [])
            if not ignore_killzone and pair_allowed_kz and kill not in pair_allowed_kz:
...
async def run_backtesting(pair=None, ignore_killzone=False):
    """
    ...
    v2.7: ignore_killzone propaga al backtest individual (investigacion sesiones)
    """
    ...
    try:
        if pair:
            log.info(f'[BT] BACKTESTING individual: {pair} (ignore_kz={ignore_killzone})')
            result = await run_backtesting_pair(pair, ignore_killzone=ignore_killzone)
            ...
            for p in active:
                result = await run_backtesting_pair(p, ignore_killzone=ignore_killzone)
...
async def api_status():
    ...
    return {
        'ok': True, 'service': 'TPDCM-IA', 'version': '2.6.0-beta-fixed17',
    ...
    try:
        ...
        for pair in get_active_pairs():
            ...
            setups.append({
                ...
                # v2.6.0-beta-fixed17: desglose detallado de los factores del score
...
async def run_backtesting_endpoint(pair: Optional[str] = None, ignore_killzone: bool = False):
    if bt_state['running']:
        return {'ok': False, 'error': 'Backtest ya en curso'}
    asyncio.create_task(run_backtesting(pair=pair, ignore_killzone=ignore_killzone))
    return {'ok': True, 'pair': pair or 'all', 'ignore_killzone': ignore_killzone,
...
async def news_endpoint():
    """v2.6.0-beta-fixed17: calendario economico de la semana (Forex Factory).
    ...
    try:
        events = await fetch_ff_calendar_full()
        today_str = now_et().strftime('%Y-%m-%d')
        out = []
        for e in events:
            # Forex Factory devuelve: title, country/currency, date (ISO), impact
            cur = (e.get('currency') or e.get('country') or '').upper()
            impact = (e.get('impact') or '').capitalize()  # High/Medium/Low/Holiday
            # date suele venir ISO tipo "2026-01-15T08:30:00-05:00"
            raw_date = e.get('date', '')
            ev_date = raw_date[:10] if len(raw_date) >= 10 else ''
            # el sistema bloquea trades solo para USD/EUR de alto impacto
            blocks_trading = (impact.lower() == 'high' and cur in ('USD', 'EUR'))
            out.append({
                'title': e.get('title', ''),
                'currency': cur,
                'impact': impact,
        ...
        currencies = sorted(set(o['currency'] for o in out if o['currency']))
        return {
            'ok': True,
            'count': len(out),
            'now_et': now_et().isoformat(),
